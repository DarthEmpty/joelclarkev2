...
export default function NavBar() {
  ...
  return (
    <nav className="flex fixed items-center p-7 w-full bg-primary-light dark:bg-primary-dark">
      <button onClick={() => setShowNav(true)} className="bg-accent rounded-full p-2 mr-5 md:collapse">
        <Hamburger />
      </button>
...

      {
        showNav ?
          <div className="absolute left-0 top-0 h-screen bg-white dark:bg-black items-center md:invisible">
            ...
            <button onClick={() => setShowNav(false)} className="bg-accent rounded-full p-2 mr-5 md:collapse">
              <LeftArrow />
            </button>
          </div>
          : null
